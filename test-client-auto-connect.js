const WebSocket = require('ws');

class TestWebSocketClient {
  constructor() {
    this.vehicles = [
      { plate: 'KEYWEST', fuel: 121.6, percentage: 50 },
      { plate: 'ALEX', fuel: 185.7, percentage: 84 },
      { plate: 'BLOEM2', fuel: 315, percentage: 100 },
      { plate: 'DURBANVILL', fuel: 200, percentage: 75 },
      { plate: 'BERGBRON', fuel: 150, percentage: 60 }
    ];
    this.step = 0;
    this.endpoints = [
      'ws://localhost:4000',
      'ws://localhost:4000/websocket',
      'ws://localhost:4000/ws',
      'ws://localhost:3000',
      'ws://localhost:3000/websocket'
    ];
    this.currentEndpoint = 0;
  }

  connect() {
    const endpoint = this.endpoints[this.currentEndpoint];
    console.log(`🧪 Test WebSocket Client - Trying: ${endpoint}`);
    
    this.ws = new WebSocket(endpoint);
    
    this.ws.on('open', () => {
      console.log(`✅ Connected to ${endpoint}`);
      console.log('🚀 Starting to send test data...\n');
      this.startSendingData();
    });
    
    this.ws.on('message', (data) => {
      console.log('📥 Received from server:', data.toString());
    });
    
    this.ws.on('close', () => {
      console.log(`❌ Disconnected from ${endpoint}`);
    });
    
    this.ws.on('error', (error) => {
      console.log(`❌ Failed to connect to ${endpoint}: ${error.message}`);
      this.tryNextEndpoint();
    });
  }

  tryNextEndpoint() {
    this.currentEndpoint++;
    if (this.currentEndpoint < this.endpoints.length) {
      setTimeout(() => this.connect(), 1000);
    } else {
      console.log('❌ All endpoints failed. Make sure your server has WebSocket support.');
      console.log('\n💡 Your server needs to handle WebSocket connections.');
      console.log('💡 Check if your server.js has WebSocket server setup.');
    }
  }

  startSendingData() {
    setInterval(() => {
      this.step++;
      
      this.vehicles.forEach((vehicle, index) => {
        let driverName = '';
        let fuelChange = 0;
        
        // Simulate fuel fill for KEYWEST every 15 steps
        if (vehicle.plate === 'KEYWEST') {
          if (this.step % 15 === 3) {
            driverName = 'POSSIBLE FUEL FILL';
            console.log(`⛽ KEYWEST: Starting fuel fill at ${vehicle.fuel.toFixed(1)}L`);
          } else if (this.step % 15 >= 4 && this.step % 15 <= 7) {
            driverName = 'POSSIBLE FUEL FILL';
            fuelChange = Math.random() * 4 + 3; // 3-7L increase during fill
          } else if (this.step % 15 === 8) {
            driverName = ''; // End fuel fill
            console.log(`⛽ KEYWEST: Fuel fill ended at ${vehicle.fuel.toFixed(1)}L`);
          } else if (this.step % 15 === 10) {
            driverName = 'ENGINE ON';
          } else if (this.step % 15 === 13) {
            driverName = 'ENGINE OFF';
          }
        }
        
        // Simulate engine operations for ALEX
        if (vehicle.plate === 'ALEX') {
          if (this.step % 20 === 2) {
            driverName = 'ENGINE ON';
          } else if (this.step % 20 === 18) {
            driverName = 'ENGINE OFF';
          } else if (this.step % 20 >= 3 && this.step % 20 <= 17) {
            driverName = 'RUNNING';
            fuelChange = -(Math.random() * 0.8 + 0.3); // Fuel consumption
          }
        }
        
        // Normal fuel consumption for others
        if (!driverName.includes('FILL') && driverName !== 'ENGINE ON') {
          fuelChange = -(Math.random() * 0.2); // Small consumption
        }
        
        vehicle.fuel += fuelChange;
        vehicle.fuel = Math.max(10, Math.min(350, vehicle.fuel));
        vehicle.percentage = Math.round((vehicle.fuel / 350) * 100);
        
        const message = {
          Plate: vehicle.plate,
          Speed: Math.floor(Math.random() * 60),
          Latitude: -26.088094 + (Math.random() - 0.5) * 0.1,
          Longitude: 27.782602 + (Math.random() - 0.5) * 0.1,
          LocTime: Date.now().toString(),
          Quality: "192.168.1." + (100 + index),
          Mileage: null,
          Pocsagstr: "",
          Head: "",
          Geozone: "Test Location, Johannesburg, South Africa",
          DriverName: driverName,
          NameEvent: "",
          Temperature: "25,405,1004,2020,0741,2021,05A5,2022,14,2023,54",
          fuel_probe_1_level: parseFloat(vehicle.fuel.toFixed(1)),
          fuel_probe_1_volume_in_tank: vehicle.fuel * 2,
          fuel_probe_1_temperature: 20 + Math.random() * 10,
          fuel_probe_1_level_percentage: vehicle.percentage,
          message_type: 405
        };
        
        if (this.ws.readyState === WebSocket.OPEN) {
          this.ws.send(JSON.stringify(message));
          
          if (driverName) {
            console.log(`📤 ${vehicle.plate}: "${driverName}" (${vehicle.fuel.toFixed(1)}L)`);
          }
        }
      });
      
    }, 3000); // Every 3 seconds
  }
}

const client = new TestWebSocketClient();
client.connect();

console.log('📋 Test Scenarios:');
console.log('- KEYWEST: Fuel fills every 15 steps');
console.log('- ALEX: Engine ON/OFF cycles');
console.log('- Others: Normal fuel consumption');
console.log('\n⏳ Trying to connect to your server...');