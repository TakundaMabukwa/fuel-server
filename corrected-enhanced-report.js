...
class CorrectedEnhancedReportGenerator {
  ...
  async generateReport(reportType = 'daily', targetDate = null) {
    try {
      ...
      const fileName = `Enhanced_Energy_Rite_${reportType}_Report_${periodName.replace(/\\s+/g, '_')}.xlsx`;
  ...
  addReportHeader(worksheet, periodName, reportType) {
    ...
    titleCell.value = 'ENHANCED FUEL REPORT SUMMARY';
  ...
  async addExpandableReportData(worksheet, sessionsData, reportType) {
    ...
    for (const site of sessionsData.sites) {
      ...
      const summaryRow = worksheet.addRow([
        hasSessions ? '+' : '-',
        site.branch,
        hasSessions ? 'Summary' : 'No Sessions',
...
async function testCorrected() {
  const generator = new CorrectedEnhancedReportGenerator();
  const daily = await generator.generateReport('daily');
  console.log('✅ Corrected enhanced report:', daily.download_url);
}
